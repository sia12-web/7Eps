-- ============================================================================
-- UPDATE GENERATE_DAILY_EDITION WITH LENS-WEIGHTED SCORING
-- ============================================================================

-- Drop existing function
DROP FUNCTION IF EXISTS generate_daily_edition(UUID);

-- Create enhanced function with lens-weighted scoring
...
BEGIN
  ...
  FOR v_user_lenses IN
    SELECT * FROM get_user_lenses(p_user_id)
  LOOP
  ...
  IF v_existing_edition > 0 THEN
    ...
    SELECT
      ...
      COALESCE((de.scoring_breakdown->i->>'hint'), 'Compatible connection'),
      (de.scoring_breakdown->i->>'score')::numeric,
      de.scoring_breakdown->i
    FROM public.daily_editions de
    CROSS JOIN generate_subscripts(de.candidate_user_ids, 1) AS i
    JOIN public.profiles p ON p.user_id = de.candidate_user_ids[i]
  ...
  WITH scored_candidates AS (
    SELECT
      ...
      (
        ...
        -
        -- Lens-based interest penalties (inverse: lower value = higher penalty)
        COALESCE(
          (
            SELECT SUM(
...
COMMENT ON FUNCTION generate_daily_edition IS 'Generates daily edition with lens-weighted scoring. Lenses adjust candidate scores based on vibe preferences without hard filtering.';
